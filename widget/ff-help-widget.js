 ...
 * Versão unificada: estilo original + todas as seções
 */
(function () {
  'use strict';
  ...
  var SECTIONS = {
    ...
    conversations: {
      ...
      tutorials: [
        ...
        { title: 'Ícones do Painel de Conversas', desc: 'Detalhes do contato, atividades, associações e tarefas.', steps: 10, time: 5, icon: '🔧', path: '/tutorials/icones-conversas/' }
      ]
    },
    calendars: {
      match: /\/calendars(\/|$)/,
      title: 'Central de Ajuda',
      subtitle: 'Aprenda a usar a seção de Calendários',
      tutorials: [
        { title: 'Como criar um calendário', desc: 'Configure agendas de atendimento e reuniões.', steps: 6, time: 3, icon: '📅', path: null },
        { title: 'Tipos de calendário', desc: 'Entenda as diferenças entre Round Robin, Coletivo e Pessoal.', steps: 5, time: 3, icon: '🔄', path: null }
      ]
    },
    opportunities: {
      match: /\/opportunities(\/|$)/,
      title: 'Central de Ajuda',
      subtitle: 'Aprenda a usar a seção de Leads',
      tutorials: [
        { title: 'Como criar sua primeira Pipeline', desc: 'Configure pipelines de vendas no estilo Kanban.', steps: 8, time: 4, icon: '🔄', path: '/tutorials/criando-pipeline/' },
        { title: 'Como aplicar Tags nas oportunidades', desc: 'Use tags para classificar e filtrar seus leads no pipeline.', steps: 6, time: 3, icon: '🏷️', path: '/tutorials/labels-oportunidades/' }
      ]
    },
    automation: {
      match: /\/(automation|workflows)(\/|$)/,
      title: 'Central de Ajuda',
      subtitle: 'Aprenda a usar Automações',
      tutorials: [
        { title: 'Visão geral de Workflows', desc: 'Entenda como funcionam as automações da plataforma.', steps: 6, time: 3, icon: '⚡', path: null }
      ]
    },
    funnels: {
      match: /\/(funnels|websites)(\/|$)/,
      title: 'Central de Ajuda',
      subtitle: 'Aprenda a usar Sites & Funis',
      tutorials: [
        { title: 'Como criar um funil', desc: 'Monte seu primeiro funil de vendas passo a passo.', steps: 8, time: 5, icon: '🌐', path: null }
      ]
    },
    fullzapp: {
      match: /\/custom-menu-link(\/|$)/,
      title: 'Central de Ajuda',
      subtitle: 'Aprenda a usar o Fullzapp',
      tutorials: [
        { title: 'Como conectar e configurar o Fullzapp', desc: 'Contrate, conecte e configure seu WhatsApp na plataforma.', steps: 10, time: 5, icon: '📱', path: '/tutorials/conectar-whatsapp-fullzapp/' }
      ]
    }
  };
  ...
  function build(section) {
    ...
    css.textContent = [
      ...
      '.ft-card.ft-soon{opacity:.55;cursor:default}',
      '.ft-card.ft-soon:hover{background:transparent}',
      ...
      '.ft-soon-badge{font-size:11px;color:#94a3b8;font-style:italic}',
    ...
    var cards = section.tutorials.map(function (t) {
      var available = !!t.path;
      var url = available ? BASE + t.path + 'index.html' : '';
      var cls = available ? 'ft-card' : 'ft-card ft-soon';
      var meta = available
        ? '<span class="ft-meta">' + t.steps + ' passos \u2022 ~' + t.time + ' min</span>'
        : '<span class="ft-soon-badge">Em breve</span>';
      return '<div class="' + cls + '"' + (available ? ' data-url="' + url + '"' : '') + '><div class="ft-ico">' + t.icon + '</div><div class="ft-nfo"><h4>' + t.title + '</h4><p>' + t.desc + '</p>' + meta + '</div></div>';
    }).join('');
    ...
    // Card clicks → open iframe (only available tutorials)
    panel.querySelectorAll('.ft-card:not(.ft-soon)').forEach(function(card) {
